...
for anglex in angle_scanx:
    for angley in angle_scany:
        for anglez in angle_scanz:
            ...
            mask.append((x_rot**2+y_rot**2<mask_radius**2)*interesting_pixel_mask)
...
for ind_image in range(5):
    ...
    for anglex in angle_scanx:
        for angley in angle_scany:    
            for anglez in angle_scanz:
                new_image1=new_image[0:sizeB,ind_progress*sizeB:(ind_progress+1)*sizeB]
                correlation_res[ind_progress,ind_image]=corr2(image_initial[mask[ind_progress]],new_image1[mask[ind_progress]])
                ind_progress=ind_progress+1

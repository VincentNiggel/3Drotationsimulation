...
path='C:\Users\vniggel\Documents\MyPhD\3Drotationsimulation\Simulation\Sim1';
cd(path)
...
contrastp=[0.2 0.9]; % contrast parameter for the matlab function imadjust
    ...
    for angley=angle_scany
        for anglez=angle_scanz
            ...
            mask(1:sizeB,1:sizeB,ind_progress)=...
                ones(sizeB,sizeB).*(x_rot.^2+y_rot.^2<mask_radius^2).*interesting_pixel_mask; %compute the mask associated to this case
...
correlation_res=zeros(number_ang,Image_number-imstep);
    parfor ind_image=1:Image_number-imstep
        cc=zeros(number_ang,1);
        ind_image
        image_interest=image_B(:,:,ind_image+imstep);
        new_image=interp2(y_pos_init,x_pos_init,image_interest,y_rot1,x_rot1); %compute all theoritical images for each given rotations
        new_image(isnan(new_image))=0; %transform the NaN values to 0.   
        correlation=0;
        ...
        for anglex=angle_scanx
            for angley=angle_scany
                for anglez=angle_scanz
                    cc(ind_progress,1)=corr2(image_initial.*mask(:,:,ind_progress),new_image(:,:,ind_progress).*mask(:,:,ind_progress)); %compute the correlation value between the two images with mask
                    ind_progress=ind_progress+1;
                end
            end
        end
        correlation_res(:,ind_image)=cc;
    end

   
[value_max ind_max]=max(correlation_res);%find maximum of correlation for each images
for ind_image=1:Image_number-imstep
    res_ang(1:3,ind_image)=ang_progress(1:3,ind_max(1,ind_image)); %save the angles corresponding to the best fit 
end
